...
bb$Meth <- factor(bb$Meth, levels=c("C25", "C2", "C25P" , "PRT", "FF"))
...
for(A in seq(5,25, by=5)) {
	for(L in seq(5,25, by=5)) {
		...
		boxplot(part.dist2~GenoError+Meth, x, 
			at=1:3 + rep(c(0,5,10,15,20),each=3),
			xlim=c(0,24),
			axes=F
		)
		box()
		axis(2, las = 2)
		axis(1, at=c(0,5,10,15,20)+2, 
			labels={if(A==25) {c("C25", "C2", "C25P" , "PRT", "FF")} else FALSE}, 
...
if(FALSE) {  # we don't really need this, I am going to add the new version to the plot made above....
  col2 <- read.table("./scores/Colony_do_over_with_new_version.txt", header = T)
  col2.new <- col2[col2$Version == "NewColony",]
  dl$c2 <- col2.new

...
version \colony~{\sc 2.0.5.7}.  We report results for both versions of the program.  Runs of the software
...
\colony~{\sc 2.0.5.7} & $d=0.02$; $m=0.02$ & $n75$ scenarios & 15 & 10,125 \\
          &                  & \lottalarge{}          & 5  &    375 \\      
\\
\colony{}-P~{\sc 2.0.5.7} & $d=0.02$; $m=0.02$ & $n75$ scenarios & 15 & 10,125 \\
            &                  & \lottalarge{}          & 5  &    375\\
 \\
...
({\em b}) loci for all methods (C25=\colony~{\sc 2.0.5.7}; C2=\colony~{\sc 2.0}; C25P=\colony-P~{\sc 2.0.5.7}; C2P=\colony-P~{\sc 2.0}; PRT=\prt{}; FF=\familyfinder; K2=\kinalyzer{} 2-allele algorithm; KC=\kinalyzer{} consensus algorithm) and all $n75$ scenarios (names 
...
\end{sidewaysfigure*}
%%
It is clear that \colony{} (both versions 2.0.5.7 and 2.0) performs without error in almost all cases.
...
in the \nosibs{} case, outperforming both versions 2.0 and 2.0.5.7
...
the new version of \colony{} (2.0.5.7) is a considerable improvement over version 2.0 in this regard, 
...
error rate, is available in Supplement~2. Across almost all of the scenarios, \colony~{\sc 2.0.5.7} performed 
...
\colony~{\sc 2.0.5.7} on all of the $n75$ simulated data sets at once.  Such a presentation loses a considerable 
...
We begin by comparing the performance of  \kinalyzer{}'s 2-allele algorithm to \colony{}~{\sc 2.0.5.7} on the 10,125 
...
\colony~{\sc 2.0.5.7} versus \kinalyzer{}'s 2-allele algorithm for 10,125 simulated
...
\prt{}'s performance on individual data sets is compared to \colony~{\sc 2.0.5.7}'s in Figure~\ref{fig:cvffsmear}{\em a,b}. 
%%%%
%\begin{equation}
...
Figures~\ref{fig:Colony2.0.5.7_v_PRT_on_nosibs} through \ref{fig:Colony2.0.5.7_v_PRT_on_onelargewh} show that 
...
solutions for erroneously-inferred sibships of size 2. \colony~{\sc 2.0.5.7} achieves the
...
than $\PDS$ for \prt{} (Figures \ref{fig:Colony2.0.5.7_v_PRT_on_sfsnoh} through 
\ref{fig:Colony2.0.5.7_v_PRT_on_slfsgwh}), it is clear that some of the errors made by \prt{} involved the 
...
errors.  Features in Figure~\ref{fig:Colony2.0.5.7_v_PRT_on_slfsgnoh} suggest that the method for handling 
...
\ref{fig:Colony2.0.5.7_v_FamilyFinder_on_allhalf}, \ref{fig:Colony2.0.5.7_v_FamilyFinder_on_allpathalf}).  
...
than \colony{}.  Figures~\ref{fig:Colony2.0.5.7_v_FamilyFinder_on_nosibs} through 
\ref{fig:Colony2.0.5.7_v_FamilyFinder_on_onelargewh} reveal that these points are predominantly from data sets 
...
\ref{fig:Colony2.0.5.7_v_ColonyP_on_nosibs} through 
\ref{fig:Colony2.0.5.7_v_ColonyP_on_onelargewh}, however, suggest that \colony-P might occasionally outperform
\colony{} in cases with many small sibling groups.  This appears to be related to the fact that, while
the {\sc 2.0.5.7} version of \colony{} is far less likely the the {\sc 2.0} version to erroneously identify
unrelated individuals as siblings, it does appear to have reduced sensitivity---that is, 
\colony{}~{\sc 2.0.5.7} appears more likely than \colony~{\sc 2.0} to fail to identify small sibling groups 
(see Figures~\ref{fig:Colony2.0.5.7_v_Colony_on_nosibs} 
through \ref{fig:Colony2.0.5.7_v_Colony_on_onelargewh}. Overall, \colony{}~{\sc 2.0.5.7}
performed as well or better than 
\colony{}-P~{\sc 2.0.5.7} in 90.4\% of the data sets on the basis of $\PDS$ and in 97.2\% of the data sets on the basis 
of $\PDST$.
...
varying in rows and columns, respectively.  For each method (C25=\colony~{\sc 2.0.5.7}; C2=\colony~{\sc 2.0}; C25P=\colony-P~{\sc 2.0.5.7}; C2P=\colony-P~{\sc 2.0}; PRT=\prt; 
...
with no half-siblings.  In these data sets, the superiority of \colony~{\sc 2.0.5.7} over its 
...
\colony~{\sc 2.0.5.7} produced very low partition distances from the \lottalarge{} data across
...
other programs or versions came close to performing as well as \colony~{\sc 2.0.5.7} across the entire 
...
\colony~{\sc 2.0.5.7} with very informative data sets (large $A$ and $L$). 
...
{\sc 2.0.5.7} and {\sc 2.0}, respectively, on the $n75$ data sets
when $A=15$.
...
2.0.5.7 are significantly lower (up to 9$\times$ lower in the $n75$ data sets) than the {\sc 2.0} version. 
...
Once again, the running time differences between \colony~{\sc 2.0.5.7} and~{\sc 2.0} 
were dramatic.  For example, with 25 loci, version {\sc 2.0} required, on average, 296 minutes, compared to
version {2.0.5.7}'s 8.2 minutes---a 36-fold reduction in runtime.  Excluding those cases when it failed 
...
\colony~{\sc 2.0.5.7} came very close to earning that distinction---only in cases where no full siblings
existed and data were limited (few loci and few alleles) was \colony~{\sc 2.0.5.7}
modestly outperformed; however
across many other conditions \colony{}~{\sc 2.0.5.7} far outperformed the other methods. Several other 
...
When full siblings existed in the sample, \colony{}~{\sc 2.0.5.7} performed best at identifying them.  
This result was consistent across scenarios (with or without half siblings) and across numbers of 
alleles and loci.  When no full siblings occurred in the sample, however, \colony{}~{\sc 2.0.5.7}
...
(and will have high likelihood of being full siblings).  In fact, version {\sc 2.0.5.7} includes
...
specificity of \colony~{\sc 2.0.5.7} comes with the cost of identifying a couple fewer full
...
splitting with either \colony~{\sc 2.0} or with \colony~{\sc 2.0.5.7}, which employs a correction
...
using \colony~{\sc 2.0.5.7}.  The correction introduced by \citet{Wang2013} has been very
successful in this regard. It should also be noted that the the {\sc 2.0.5.7} version is signficantly
faster than {\sc 2.0}---up to 36 times faster on the \lottalarge{} data, making \colony{} faster than all
the other methods save \familyfinder{} and \colony-P~{\sc 2.0.5.7} in that scenario.  
...
Although \colony~{\sc 2.0.5.7} is orders of magnitude faster than \colony~{\sc 2.0}, \colony-P~{\sc 2.0.5.7} \citep{Wang2012pairwise} is considerably faster than \colony{}~{\sc 2.0.5.7} on data sets with
many individuals, and it performs as well, relative to \colony{}~{\sc 2.0.5.7}, as any of the other methods.
